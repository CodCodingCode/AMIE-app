...
class MedicalScannerApp:
    ...
    def submit_response(self):
        ...
        try:
            ...
            response = requests.post(
                f"{CLINICAL_AI_SERVER}/continue_conversation",
                json=payload,
                timeout=30
            )
...
def main():
    ...
    try:
        response = requests.get(f"{IMAGE_ANALYSIS_SERVER}/", timeout=5)
        print("✅ Image analysis server connected")
    except:
        print(f"❌ Cannot connect to image analysis server at {IMAGE_ANALYSIS_SERVER}")
        print("Please update IMAGE_ANALYSIS_SERVER variable with correct IP")
        
    try:
        response = requests.get(f"{CLINICAL_AI_SERVER}/", timeout=5)
        print("✅ Clinical AI server connected")
    except:
        print(f"❌ Cannot connect to clinical AI server at {CLINICAL_AI_SERVER}")
        print("Please update CLINICAL_AI_SERVER variable with correct IP")
