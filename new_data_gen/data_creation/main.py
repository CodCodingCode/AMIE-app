...
class FamilyHistoryImpact(BaseModel):
    inheritance_pattern: str = Field(
        description="How the disease is inherited (autosomal dominant, recessive, X-linked, etc.)"
    )
    risk_increase: str = Field(description="How family history increases risk")
    age_of_onset_influence: str = Field(
        description="How family history affects age of onset"
    )
    severity_influence: str = Field(
        description="How family history affects disease severity"
    )
    screening_recommendations: str = Field(
        description="Screening recommendations for family members"
    )
...
class MedicalInformation(BaseModel):
    ...
    diagnosis_methods: List[str] = Field(
        description="List of diagnostic methods and tests"
    )
    ...
    family_history_impact: FamilyHistoryImpact = Field(
        description="Impact of family history on the disease"
    )
    hereditary_factors: List[str] = Field(
        description="List of hereditary and genetic factors"
    )
...
class MedicalInfoSystem:
    def __init__(self):
        ...
        # File paths
        self.disease_db_path = "/Users/owner/Downloads/coding projects/AMIE-app/new_data_gen/data_creation/balanced_diseases_sample2.csv"
    ...
    def _get_llm_extraction_strategy(self, disease: str) -> LLMExtractionStrategy:
        ...
        return LLMExtractionStrategy(
            llm_config=LLMConfig(
                provider="openai/gpt-4o-mini", api_token=self.openai_api_key
            ),
...
Be thorough and extract specific, detailed information. If certain information is not available, use your medical knowledge to provide accurate information for the field.""",
        ...
        try:
            ...
            for chunk in pd.read_csv(self.disease_db_path, chunksize=chunk_size):
                ...
                if term_column is None:
                    ...
                    logger.warning(
                        f"Using first column '{term_column}' as disease terms"
                    )
                else:
                ...
                for _, row in chunk.iterrows():
                    ...
                    if (
                        term
                        and len(term) > 3
                        and term.lower() not in ["nan", "null", "none"]
                        and term not in processed_terms
                    ):
    ...
    async def search_medical_websites(
        self, disease: str, worker_id: int = 0
    ) -> List[str]:
        ...
        try:
            ...
            for query in queries:
                try:
                    ...
                    logger.warning(
                        f"Worker {worker_id}: Search failed for query '{query}': {search_e}"
                    )
    ...
    async def crawl_multiple_websites(
        self, disease: str, urls: List[str], worker_id: int = 0
    ) -> Dict[str, Any]:
        ...
        async with AsyncWebCrawler(config=browser_config) as crawler:
            ...
            for i, url in enumerate(urls):
                try:
                    logger.info(
                        f"Worker {worker_id}: Crawling website {i+1}/{len(urls)}: {url}"
                    )
                    ...
                    if result.success and result.extracted_content:
                        try:
                                ...
                                if isinstance(json_data, list):
                                    ...
                                    for item in json_data:
                                        if (
                                            ...
                                            and item.get("error") != True
                                            and "disease_name" in item
                                        ):
                                            extracted_data = item
                                            break
                                    ...
                                    if extracted_data is None:
                                        logger.warning(
                                            f"Worker {worker_id}: No valid medical data found in extraction results from {url}"
                                        )
                                        continue
                                    ...
                                    logger.warning(
                                        f"Worker {worker_id}: Unexpected JSON structure from {url}: {type(json_data)}"
                                    )
                                    continue
                            else:
                                logger.warning(
                                    f"Worker {worker_id}: Unexpected content type from {url}: {type(result.extracted_content)}"
                                )
                                continue
                            ...
                            logger.info(
                                f"Worker {worker_id}: Successfully extracted data from {url}"
                            )

                        except (json.JSONDecodeError, KeyError, TypeError) as e:
                            logger.warning(
                                f"Worker {worker_id}: Data parsing error for {url}: {e}"
                            )
                    else:
                        logger.warning(
                            f"Worker {worker_id}: Failed to crawl or extract from {url}"
                        )
        ...
        if not all_extracted_data:
            return {
                "error": "No data extracted from any website",
                "disease_name": disease,
            }

        # Combine data from multiple websites
        combined_data = self._combine_extracted_data(
            disease, all_extracted_data, successful_urls
        )
        logger.info(
            f"Worker {worker_id}: Successfully combined data from {len(successful_urls)} websites for {disease}"
        )

        return combined_data

    def _combine_extracted_data(
        self, disease: str, extracted_data_list: List[Dict], urls: List[str]
    ) -> Dict[str, Any]:
        ...
        for field in text_fields:
            ...
            for data in extracted_data_list:
                if (
                    ...
                    and len(data[field]) > len(longest_text)
                ):
        ...
        for field in family_history_fields:
            ...
            for data in extracted_data_list:
                if (
                    "family_history_impact" in data
                    and isinstance(data["family_history_impact"], dict)
                    and field in data["family_history_impact"]
                    and len(str(data["family_history_impact"][field]))
                    > len(longest_text)
                ):
    ...
    async def search_and_crawl_multiple_sites(
        self, disease: str, worker_id: int = 0
    ) -> Dict[str, Any]:
        """Complete workflow: search for websites and crawl multiple sites with built-in LLM extraction"""
        logger.info(
            f"Worker {worker_id}: Processing {disease} with multiple website crawling"
        )
        ...
        for attempt in range(min(3, len(self.tavily_keys))):
            try:
                if attempt > 0:
                    ...
                    logger.info(
                        f"Worker {worker_id}: Trying with Tavily key {key_index}"
                    )
                ...
                logger.error(
                    f"Worker {worker_id}: Attempt {attempt + 1} failed for {disease}: {e}"
                )
                if attempt < min(2, len(self.tavily_keys) - 1):
                    ...
                    logger.error(
                        f"Worker {worker_id}: All attempts failed for {disease}"
                    )
        ...
        try:
            with self.csv_lock:
                ...
                row_data = {
                    ...
                    "treatment_options": "; ".join(
                        medical_info.get("treatment_options", [])
                    ),
                    "diagnosis_methods": "; ".join(
                        medical_info.get("diagnosis_methods", [])
                    ),
                    ...
                    "inheritance_pattern": family_history.get(
                        "inheritance_pattern", ""
                    ),
                    "family_risk_increase": family_history.get("risk_increase", ""),
                    "age_of_onset_influence": family_history.get(
                        "age_of_onset_influence", ""
                    ),
                    "severity_influence": family_history.get("severity_influence", ""),
                    "screening_recommendations": family_history.get(
                        "screening_recommendations", ""
                    ),
                    "hereditary_factors": "; ".join(
                        medical_info.get("hereditary_factors", [])
                    ),
                    "genetic_risk_assessment": medical_info.get(
                        "genetic_risk_assessment", ""
                    ),
                    "processing_timestamp": medical_info.get(
                        "processing_timestamp", datetime.now().isoformat()
                    ),
                ...
                if os.path.exists(self.results_csv_path):
                    new_row_df.to_csv(
                        self.results_csv_path, mode="a", header=False, index=False
                    )
                else:
                    new_row_df.to_csv(self.results_csv_path, index=False)

                logger.info(
                    f"Saved {medical_info.get('disease_name')} to CSV ({medical_info.get('websites_crawled', 0)} websites)"
                )
    ...
    def process_disease(self, disease: str, worker_id: int = 0) -> Dict[str, Any]:
        ...
        try:
            medical_info = loop.run_until_complete(
                self.search_and_crawl_multiple_sites(disease, worker_id)
            )
        finally:
            loop.close()
        ...
        if "error" not in medical_info:
            ...
            logger.warning(
                f"Worker {worker_id}: Skipping save for {disease} due to processing error"
            )

        return medical_info

    def process_diseases_worker(
        self, diseases: List[str], worker_id: int, progress_queue: Queue
    ) -> List[Dict[str, Any]]:
        ...
        for i, disease in enumerate(diseases):
            try:
                logger.info(
                    f"Worker {worker_id}: Processing disease {i+1}/{len(diseases)}: {disease}"
                )
    ...
    def process_all_diseases_parallel(self, num_workers: int = 12, limit: int = None):
        ...
        print(
            f"Starting parallel processing of {len(diseases)} diseases{limit_text} with {num_workers} workers..."
        )
        print(
            "NOTE: Using Crawl4AI's built-in LLM extraction with multiple website crawling per disease"
        )
            ...
            while processed_count < total_diseases:
                try:
                    ...
                    if processed_count % 10 == 0 or processed_count == total_diseases:
                        print(
                            f"Progress: {processed_count}/{total_diseases} diseases processed ({processed_count/total_diseases*100:.1f}%)"
                        )
                except:
                    continue
        ...
        with ThreadPoolExecutor(max_workers=actual_workers) as executor:
            future_to_worker = {
                executor.submit(
                    self.process_diseases_worker, chunk, i, progress_queue
                ): i
            ...
            for future in as_completed(future_to_worker):
                ...
                try:
                    ...
                    logger.info(
                        f"Worker {worker_id} completed successfully with {len(worker_results)} results"
                    )
                except Exception as e:
        ...
        print(
            f"Average websites per disease: {total_websites/successful if successful > 0 else 0:.1f}"
        )
        print(f"Processing time: {processing_time:.2f} seconds")
        print(
            f"Average time per disease: {processing_time/len(all_results):.2f} seconds"
        )
