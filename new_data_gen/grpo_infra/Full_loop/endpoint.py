...
class HuggingFaceInference:
    def __init__(self, endpoint_url, api_token, model_name=""):
    ...
    def generate(self, prompt, max_new_tokens=800):
        ...
        try:
            ...
            print(f"❌ API request failed for {self.model_name}: {e}")
...
model_client = HuggingFaceInference(DOCTOR_ENDPOINT_URL, HF_TOKEN, "Doctor Model")
patient_model = HuggingFaceInference(PATIENT_ENDPOINT_URL, HF_TOKEN, "Patient Model")
...
def run_conversation():
    ...
    vignette, condition = get_random_vignette()  # ADD THIS LINE

    print("\n" + "=" * 60)
    print("🏥 Starting Clinical Conversation")
    print("=" * 60)
    print(f"🩺 Disease/Condition: {condition}")
    print(f"📋 Using vignette: {vignette[:100]}...")  # ADD THIS LINE

    question = "Doctor: What brings you in today?"
    ...
    if "ANSWER: " in patient_response:
        patient_response = patient_response.split("ANSWER: ")[
            -1
        ].strip()  # ADD THIS LINE
...
if __name__ == "__main__":
    ...
    print(f"🔗 Doctor Endpoint: {DOCTOR_ENDPOINT_URL}")
    print(f"👤 Patient Endpoint: {PATIENT_ENDPOINT_URL}")
