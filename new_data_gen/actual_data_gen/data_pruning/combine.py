...
for i, (instruction, filename) in enumerate(files.items()):
    ...
    with open(filename, "r") as f:
        ...
        for entry in entries:
            # Use "analysis" for the second file, "output" for others
            output_column = "analysis" if i == 1 else "output"

            combined_data.append(
                {
                    "instruction": instruction,
                    "input": re.sub(r"^\s*\d+\.\s*", "", entry.get("input", "")),
                    "output": entry.get(output_column, ""),
                }
            )
