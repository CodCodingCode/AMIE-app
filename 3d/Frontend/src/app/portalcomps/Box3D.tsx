...
const Box3D = ({ initialPosition = [0, 0, -30], onZoomStart, onBoxClicked, onAnimationStateChange, getTextAnimationControls }: Box3DProps) => {
  ...
  const { isBlueboxTriggered, resetBlueboxTrigger } = useBlueboxAnimation();
  ...
  useEffect(() => {
    if (isBlueboxTriggered && !isZooming.current && !hasZoomedOnce) {
      handleZoom();
      setHasZoomedOnce(true);
    }
  }, [isBlueboxTriggered, hasZoomedOnce]);
  ...
  const handleZoom = () => {
    ...
    if (window.scrollY > 5) {
      ...
      setTimeout(() => {
        window.removeEventListener('scroll', handleScrollToTop);
        if (window.scrollY <= 5 && !isZooming.current) {
          handleZoom();
        }
      }, 2000);
      return;
    ...
    materialRefs.current.forEach((material) => {
      if (material !== cube005MaterialRef.current) {
        tl.to(material, {
          opacity: 0,
          duration: 0.3,
          ease: 'power1.in',
    ...
    if (boxRef.current) {
      tl.to(boxRef.current.rotation, {
        y: boxRef.current.rotation.y - Math.PI / 2,
        duration: 1,
        ease: 'power2.inOut'
      }, 0);
    }
  ...
  const handleClick = () => {
    if (isBlueboxTriggered && !hasZoomedOnce) return;
    if (isZooming.current) return;
