...
def validate_diagnosis(diagnosis: str, diseases_to_avoid: List[str]) -> bool:
    ...
    for disease in diseases_to_avoid:
        if disease.lower() in diagnosis_lower or diagnosis_lower in disease.lower():
            print(
                f"REJECTED: '{diagnosis}' is too similar to existing diagnosis '{disease}'"
            )
            return False
    return True


def generate_unique_diseases(num_diseases: int = 5000) -> List[str]:
        ...
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert physician. Generate unique medical diagnoses with ICD-10 codes.",
                    },
            ...
            for disease in new_diseases:
                if disease not in diseases:
                    ...
                    print(
                        f"Generated unique disease {len(diseases)}/{num_diseases}: {disease}"
                    )
...
2. The vignette must include appropriate demographics (age and sex) that make clinical sense for this specific diagnosis
...
Generate a case following the exact format above, ensuring the demographics are clinically appropriate for {disease}."""
    ...
    for attempt in range(3):
        try:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert physician. You must return a valid JSON object with exactly two fields: 'doctor_vignette' and 'actual_diagnosis'.",
                    },
                    {"role": "user", "content": prompt},
                ],
                temperature=0.7,
        ...
        time.sleep(1)
    return {
        "doctor_vignette": "Failed to generate vignette.",
        "actual_diagnosis": disease,
    }


def load_diseases_from_malacard(filename="malacard-diseases.json") -> List[str]:
    """
    Load diseases from the malacard-diseases.json file.

    Args:
        filename (str): Path to the JSON file containing diseases.

    Returns:
        List[str]: A list of disease names extracted from the file.
    """
    try:
        with open(filename, "r", encoding="utf-8") as f:
            diseases_data = json.load(f)
            # Extract the 'disease' field from each dictionary in the JSON
            diseases = [
                entry["disease"].strip()
        ...
        print(f"Error loading diseases from {filename}: {e}")
        return []


def generate_cases_parallel(
    num_cases=500, max_workers=5, diseases_file="malacard-diseases.json"
):
    """
    Generate cases in parallel using diseases from malacard-diseases.json.

    Args:
        num_cases (int): Total number of cases to generate.
        max_workers (int): Number of threads to use for parallel processing.
        diseases_file (str): Path to the JSON file containing diseases.
    """
    ...
    print(
        f"Found {start_idx} existing cases. Generating {num_cases - start_idx} new cases..."
    )

    # Load diseases from malacard-diseases.json
    diseases = load_diseases_from_malacard(diseases_file)
    if not diseases:
        print("No diseases found in the provided file. Exiting.")
        return

    # Limit diseases to the number of cases needed
    diseases_to_use = diseases[start_idx:num_cases]
    print(f"Using {len(diseases_to_use)} diseases for case generation.")

    # Generate cases in parallel
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        futures = [
            executor.submit(generate_case_for_disease, disease)
            for disease in diseases_to_use
        ]
...
if __name__ == "__main__":
    generate_cases_parallel(num_cases=5000, max_workers=5)
